...
public class KeelRandomHelper {
    ...
    public String generateRandomString(int length) {
        ...
        VertxContextPRNG prng = this.getPRNG();
     ...
     * @return 唯一随机字符串列表
     */
    public List<String> generateUniqueRandomStrings(int count, int length) {
        if (count <= 0 || length <= 0) {
        ...
        while (uniqueStrings.size() < count) {
            uniqueStrings.add(this.generateRandomString(length));
        }
