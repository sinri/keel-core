...
public class Config1 {
    ...
    public static void main(String[] args) {
        ...
        Keel.getIssueRecorder().fatal(x -> x.message("cachedConfig: ").context(cachedConfig));

        configRetriever.getConfig()
                .compose(config -> {
                    Keel.getIssueRecorder().fatal(x -> x.message("Retrieved: ").context(config));
                    return Future.succeededFuture();
                });

        Keel.getVertx().setTimer(3000L, v -> {
            JsonObject cachedConfig2 = configRetriever.getCachedConfig();
            Keel.getIssueRecorder().fatal(x -> x.message("cachedConfig2: ").context(cachedConfig2));
            Keel.close();
        });
    }
}
