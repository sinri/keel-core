...
public class CutterUnitTest extends KeelUnitTest {
    ...
    public void testCutterOnString() {
        async(() -> {
            ...
            IntravenouslyCutterOnString cutterOnString = new IntravenouslyCutterOnString(s -> {
            ...
            return Keel.asyncCallStepwise(4, i -> {
                           ...
                           cutterOnString.acceptFromStream(Buffer.buffer(substring));
                           return Keel.asyncSleep(1_000L);
                       })
                       .compose(v -> {
                           cutterOnString.stopHere();
                           return cutterOnString.waitForAllHandled();
                       })
                       .compose(v -> {
                           return Keel.asyncSleep(1_000L);
                       })
