...
class NetUtilsTest extends KeelJUnit5Test {

    public NetUtilsTest(Vertx vertx) {
        super(vertx);
    }

    private NetUtils getHelper() {
    ...
    void testConvertIPv4ToNumber_NormalIP() {
        ...
        getHelper();
        Long result = NetUtils.convertIPv4ToNumber(ipv4);
    ...
    void testConvertIPv4ToNumber_PrivateNetworks() {
        // 10.0.0.0
        getHelper();
        Long result1 = NetUtils.convertIPv4ToNumber("10.0.0.0");
        assertNotNull(result1);
        assertEquals(167772160L, result1.longValue());
        
        // 172.16.0.0
        getHelper();
        Long result2 = NetUtils.convertIPv4ToNumber("172.16.0.0");
        assertNotNull(result2);
        assertEquals(2886729728L, result2.longValue());
        
        // 192.168.0.0
        getHelper();
        Long result3 = NetUtils.convertIPv4ToNumber("192.168.0.0");
    ...
    void testConvertIPv4ToNumber_InvalidIP() {
        ...
        for (String invalidIP : invalidIPs) {
            getHelper();
            Long result = NetUtils.convertIPv4ToNumber(invalidIP);
        ...
        getHelper();
        Long nullResult = NetUtils.convertIPv4ToNumber(null);
    ...
    void testConvertNumberToIPv4_NormalConversion() {
        ...
        getHelper();
        String result = NetUtils.convertNumberToIPv4(number);
        assertNotNull(result);
    ...
    void testConvertIPv4ToAddressBytes_Normal() {
        ...
        getHelper();
        byte[] result = NetUtils.convertIPv4ToAddressBytes(ipv4AsLong);
    ...
    void testConvertIPv4ToAddressBytes_BoundaryValues() {
        ...
        for (long value : testValues) {
            getHelper();
            byte[] result = NetUtils.convertIPv4ToAddressBytes(value);
    ...
    void testGetLocalHostInfo() {
        ...
        // 测试获取本地规范主机名
        getHelper();
        String canonicalName = NetUtils.getLocalHostCanonicalName();
        // 可能为null如果无法解析本地主机名
    ...
    void testIPConversionRoundtrip() {
        ...
        for (String originalIP : testIPs) {
            getHelper();
            Long number = NetUtils.convertIPv4ToNumber(originalIP);
            if (number != null) {
                // 测试字节数组转换
                getHelper();
                byte[] bytes = NetUtils.convertIPv4ToAddressBytes(number);
    ...
    void testIPConversionPerformance() {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            getHelper();
            Long number = NetUtils.convertIPv4ToNumber(ip);
            if (number != null) {
                getHelper();
                byte[] bytes = NetUtils.convertIPv4ToAddressBytes(number);
                assertNotNull(bytes);
    ...
    void testEdgeCasesAndErrorHandling() {
        // 测试各种边界情况 - 真正无效的输入
        getHelper();
        assertNull(NetUtils.convertIPv4ToNumber("invalid"));
        getHelper();
        assertNull(NetUtils.convertIPv4ToNumber("999.999.999.999"));
        getHelper();
        assertNull(NetUtils.convertIPv4ToNumber("192.168.1.1.1"));
        ...
        getHelper();
        Long localhostResult = NetUtils.convertIPv4ToNumber("127.0.0.1");
        assertNotNull(localhostResult); // This should return 2130706433
        assertEquals(2130706433L, localhostResult.longValue());
        
        // 测试数字转IP的边界情况
        getHelper();
        assertNull(NetUtils.convertNumberToIPv4(-1));
        getHelper();
        String tooLargeResult = NetUtils.convertNumberToIPv4(4294967296L); // 2^32
        assertNotNull(tooLargeResult); // Returns "0.0.0.0"
        assertEquals("0.0.0.0", tooLargeResult);
        
        // 测试字节数组转换的边界情况
        getHelper();
        byte[] result = NetUtils.convertIPv4ToAddressBytes(Long.MAX_VALUE);
        assertNotNull(result);
        assertEquals(4, result.length);
    }
} 