package io.github.sinri.keel.core.servant.intravenous.legacy;
...
abstract public class KeelIntravenousBase<T> extends KeelVerticleImpl {
    ...
    public void add(T t) {
        ...
        queue.add(t);
        this.intravenousLogger.info("adding " + t + " to intravenous");
        Promise<Void> currentInterrupt = getCurrentInterrupt();
        if (currentInterrupt != null) {
            currentInterrupt.tryComplete();
            this.intravenousLogger.info("interrupting");
        }
    }
    ...
    protected Future<Void> startVerticle() {
        ...
        Keel.asyncCallRepeatedly(repeatedlyCallTask -> {
            this.intravenousLogger.info("in KeelIntravenousBase first repeatedly call");
            this.interruptRef.set(null);

            return Keel.asyncCallRepeatedly(routineResult -> {
                           this.intravenousLogger.info("in KeelIntravenousBase second repeatedly call");
                           ...
                           if (buffer.isEmpty()) {
                               routineResult.stop();
                               this.intravenousLogger.info("in KeelIntravenousBase second repeatedly call stop as no drops");
                           ...
                           return Future.succeededFuture()
                                        .compose(v -> {
                                            this.intravenousLogger.info("in KeelIntravenousBase second repeatedly call handle one drop");
                       ...
                       .andThen(ar -> {
                           ...
                           Keel.asyncSleep(sleptTime(), getCurrentInterrupt())
                               .andThen(slept -> {
                                   ...
                                   this.intravenousLogger.info("in KeelIntravenousBase first repeatedly call stop as interrupted");
                               });
                       });
        });
    ...
    protected KeelIssueRecorder<KeelEventLog> buildIntravenousIssueRecorder() {
        return KeelIssueRecorder.buildSilentIssueRecorder();
    }
